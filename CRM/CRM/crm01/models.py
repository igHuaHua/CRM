...
class Customer(models.Model):
    ...
    def show_status(self):
        ...
        return '<span style="background-color: {};color:white;padding: 5px">{}</span>'.format(color_dict[self.status],self.get_status_display())
    def show_class(self):
        return ' | '.join([str(i) for i in self.class_list.all()])

    def __str__(self):
        return '{}<{}>'.format(self.qq_name,self.qq)
